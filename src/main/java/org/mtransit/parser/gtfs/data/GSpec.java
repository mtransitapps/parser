...
public class GSpec {
	...
	@Nullable
	public ArrayList<GTrip> getTrips(@Nullable String optRouteId) {
		if (optRouteId != null) {
	...
	public List<GStopTime> getStopTimes(@SuppressWarnings("unused") @Nullable Long optMRouteId,
		...
		if (optGTripId != null) {
			return DBUtils.selectStopTimes(optGTripId);
		}
			...
			for (GStopTime gStopTime : tripStopTimes) {
				try {
					if (gStopTimeIncInSec.containsKey(gStopTime.getUID())) {
						MTLog.log("stop time UID '%s' already in list with value '%s'!", gStopTime.getUID(),
	...
	@NotNull
	private Calendar getDepartureTimeCal(@NotNull Calendar calendar, SimpleDateFormat gDateFormat, long mRouteId, GStopTime gStopTime) throws ParseException {
