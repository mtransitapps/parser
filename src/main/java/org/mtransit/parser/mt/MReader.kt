...
object MReader {
    ...
    fun readFirstLastDepartures(fileBase: String): Pair<Int?, Int?>? {
        try {
            val resDirName = if ("current_".equals(fileBase, ignoreCase = true)) {
                "$RES-current"
            ...
            val gtfsRtsValuesGenXml = "$MAIN_SRC_DIR/$resDirName/$VALUES/${fileBase}$GTFS_RTS_VALUES_GEN_XML"
            ...
            val commonRtsValuesGenXml = "$MAIN_SRC_DIR/$RES/$VALUES/$GTFS_RTS_VALUES_GEN_XML"
            ...
            MTLog.logNonFatal(e, "Error while reading first/last departures!")
            return null
        }
    }
}